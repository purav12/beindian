...
function centerPopupForcerefund() {

    var windowWidth = document.documentElement.clientWidth;
    var windowHeight = document.documentElement.clientHeight;
    var popupHeight = $("#popupContactForcerefund").height();
    var popupWidth = $("#popupContactForcerefund").width();
    $("#popupContactForcerefund").css({
        "position": "fixed",
...
function loadPopupForcerefund() {
    if (popupStatus == 0) {
        $("#backgroundPopup").css({
            "opacity": "0.7"
        });
        $("#backgroundPopup").fadeIn("slow");
        $("#popupContactForcerefund").fadeIn("slow");
        popupStatus = 1;
    }
}

function disablePopupForcerefund() {
    if (popupStatus == 1) {
        $("#backgroundPopup").fadeOut("slow");
        $("#popupContactForcerefund").fadeOut("slow");
        $("#popupContactForcerefund").fadeOut("slow");
        popupStatus = 0;
    }
